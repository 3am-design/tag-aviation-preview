...
export function GuestField({ value, onChange }) {
  ...
  return <div className="passenger-field" ref={anchor}><label htmlFor={id}>GUESTS</label><button ref={trigger} id={id} className="field-trigger" type="button" aria-label="Number of guests" aria-describedby={`${id}-value`} aria-haspopup="dialog" aria-expanded={open} aria-controls={open ? `${id}-guests` : undefined} onClick={() => setOpen(!open)}><FieldIcon name="people" /><span id={`${id}-value`}>{count} {count === 1 ? 'guest' : 'guests'}</span><FieldIcon name="chevron" /></button>{open && <FloatingPanel anchor={anchor} onClose={close} width={320} className="guest-popover" role="dialog" aria-label="Choose number of guests" id={`${id}-guests`}><div className="popover-heading"><span>Your guests</span><button type="button" className="popover-close" autoFocus aria-label="Close guest selector" onClick={() => close(true)}><FieldIcon name="close" /></button></div><div className="guest-picker"><span>Passengers</span><div><button type="button" aria-label="Remove one guest" disabled={count <= 1} onClick={() => onChange(count - 1)}><FieldIcon name="minus" /></button><output aria-live="polite">{count}</output><button type="button" aria-label="Add one guest" disabled={count >= 30} onClick={() => onChange(count + 1)}><FieldIcon name="plus" /></button></div></div><button type="button" className="popover-done" onClick={() => close(true)}>Done</button></FloatingPanel>}</div>;
}
