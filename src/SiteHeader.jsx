...
export default function SiteHeader({ logo, page, onPlan }) {
  ...
  useEffect(() => {
    ...
    const resize = () => { setExpanded(null); setMobileOpen(false); setCompact(!desktop.matches); };
  ...
  return <header className="site-header" data-tag-intro="header" ref={header} onKeyDown={event => {
    ...
    <nav id={`${id}-navigation`} aria-label="Main navigation" aria-hidden={compact && !mobileOpen ? true : undefined} inert={compact && !mobileOpen} className={mobileOpen ? 'open' : ''}>
      {groups.map((group, index) => <div key={group.title} className={`nav-item ${group.wide ? 'nav-item-wide' : ''}`} onPointerEnter={event => { if (event.pointerType === 'mouse' && window.matchMedia('(min-width: 960px)').matches) setExpanded(index); }} onPointerLeave={event => { if (event.pointerType === 'mouse' && !event.currentTarget.contains(document.activeElement)) setExpanded(null); }}>
        <button type="button" className="nav-trigger" data-tag-intro-item data-menu-trigger={index} aria-expanded={expanded === index} aria-controls={`${id}-submenu-${index}`} onClick={event => setExpanded(current => event.detail > 0 && window.matchMedia('(min-width: 960px)').matches ? index : current === index ? null : index)} onKeyDown={event => { if (event.key === 'ArrowDown') { event.preventDefault(); setExpanded(index); } }}>{group.title}<Chevron /></button>
        <div id={`${id}-submenu-${index}`} aria-hidden={expanded !== index} inert={expanded !== index} className={`nav-submenu ${group.wide ? 'nav-submenu-wide' : ''} ${expanded === index ? 'is-open' : ''}`}><div className="nav-submenu-inner wrap">
          ...
          {group.columns.map((column, c) => <section key={c} className="nav-column" style={{ '--tag-menu-order': c + 1 }}>{column.title && <h2>{column.title}</h2>}<ul>{column.items.map(item => <li key={item}><span role="link" aria-disabled="true">{item}</span></li>)}</ul></section>)}
        </div></div>
      </div>)}
      <button type="button" className="nav-inactive" data-tag-intro-item disabled>Private charter</button>
      <button type="button" className="nav-inactive" data-tag-intro-item disabled>The TAG difference</button>
      <button type="button" className="nav-contact" data-tag-intro-item disabled>Contact</button>
    </nav>
    <div className="header-actions" data-tag-intro-item><button className="button primary header-cta" onClick={() => { setMobileOpen(false); setExpanded(null); onPlan(); }}>Plan your flight <svg className="tag-action-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 12h15M13 5l7 7-7 7" /></svg></button><button ref={mobileTrigger} className="menu-toggle" aria-label={mobileOpen ? 'Close menu' : 'Open menu'} aria-expanded={mobileOpen} aria-controls={`${id}-navigation`} onClick={() => { setMobileOpen(!mobileOpen); setExpanded(null); }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" aria-hidden="true"><path d={mobileOpen ? 'm5 5 14 14M19 5 5 19' : 'M4 8h16M4 16h16'} /></svg></button></div>
  </header>;
}
